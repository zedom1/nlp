...
class RCModel(object):
    ...
    def _setup_placeholders(self):
        ...
        self.fuse_value = tf.placeholder(tf.float32)
        self.match_value = tf.placeholder(tf.float32)
    ...
    def _match(self):
        ...
        self.match_value = tf.reduce_mean(self.match_p_encodes)
        if self.use_dropout:
    ...
    def evaluate(self, eval_batches, result_dir=None, result_prefix=None):
        ...
        for b_itx, batch in enumerate(eval_batches):
            ...
            start_probs, end_probs, loss, fuse_value, context2question_attn, question2context_attn, match_value = self.sess.run([
                self.start_probs, self.end_probs, self.loss, self.fuse_value, self.context2question_attn,
                self.question2context_attn, self.match_value], feed_dict)
            ...
            for sample, start_prob, end_prob in zip(batch['raw_data'], start_probs, end_probs):
                ...
                if 'answers' in sample:
                    ...
                    pred_answers.append({'question_id': sample['question_id'],
                                         ...
                                         'fuse_value': str(fuse_value),
                                         'q2c_match_score': str(q2c_match_score),
                                         'c2q_match_score': str(c2q_match_score),
    ...
    def find_best_answer(self, sample, start_prob, end_prob, padded_p_len, context2question_attn, question2context_attn):
        ...
        q2c_match_score, c2q_match_score = 0, 0
        total_score = 0
        ...
        if best_p_idx is None or best_span is None:
            ...
            c2q_total, q2c_total, c2q_num, q2c_num = 0, 0, 0, 0
            for c2q_mat in context2question_attn[best_p_idx][best_span[0]: best_span[1] + 1]:
                for c2q_val in c2q_mat:
                    c2q_total += c2q_val
                    c2q_num += 1
            for q2c_mat in question2context_attn[best_p_idx][best_span[0]: best_span[1] + 1]:
                for q2c_val in q2c_mat:
                    q2c_total += q2c_val
                    q2c_num += 1
            best_answer = ''.join(
                sample['passages'][best_p_idx]['passage_tokens'][best_span[0]: best_span[1] + 1])
            q2c_match_score = q2c_total/q2c_num
            c2q_match_score = c2q_total/c2q_num
            seg_answers = sample['passages'][best_p_idx]['passage_tokens'][best_span[0]: best_span[1] + 1]
        best_score = best_score/total_score
        return best_answer, seg_answers, best_score, q2c_match_score, c2q_match_score
